...
import (
	...
	model "github.com/gorest1/models"
...
func CreateTodoHandler(db *gorm.DB) http.HandlerFunc {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			w.Header().Set("content-type", "application/json")
			w.WriteHeader(http.StatusInternalServerError)
			io.WriteString(w, `{"Error": "Failed copy file with error"}`)
			return
		}
		...
		log.WithFields(log.Fields{"Description": description, "ImageURL": fullPathname}).Info("Succes Add new todo Item")
...
func UpdateTodoHandler(db *gorm.DB) http.HandlerFunc {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		log.WithFields(log.Fields{"ID": id, "Description": description, "IsCompleted": isCompleted}).Info("Success updating todo")
...
func DeleteTodoHandler(db *gorm.DB) http.HandlerFunc {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		log.WithFields(log.Fields{"ID": id}).Info("Success deleted todo item")
